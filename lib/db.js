...
const getUsers = (request, response) => {
	return pool
		.query('SELECT * FROM social_workers ORDER BY id ASC').then((error, results) => {
...
const loginQuery = (request, response) => {
	...
	if (username && password) {
		return pool
			...
			.query('SELECT * FROM social_workers WHERE username = $1 AND password = $2', [username, password]).then((error, results) => {
